...
export function BusinessReportsClient() {
  ...
  const loadReports = useCallback(
    async (silent = false) => {
      ...
      setError(null);
      try {
        const res = await fetch(`/api/admin/reports/dashboard?${dashboardQuery}`);
  ...
  async function backfillCostSnapshots() {
    ...
    try {
      const res = await fetch(`/api/admin/reports/backfill-cost-snapshots?${dashboardQuery}`, {
        method: "POST",
      });
  ...
  return (
    <div className="min-w-0">
      ...
      <div className="mb-6 flex flex-wrap gap-2 border-b border-zinc-200 pb-1">
        {reportTabs.map((tab) => (
          <button
            key={tab.id}
            type="button"
            ...
            className={`rounded-t-lg px-4 py-2 text-sm font-medium transition-colors ${
              activeTab === tab.id
                ? "border border-b-white border-zinc-200 bg-white text-rose-700"
      ...
      {activeTab === "purchase" ? (
        ...
        <div className="space-y-6">
                    ...
                    <div key={label}>
                      ...
                      <div className="h-2 rounded-full bg-zinc-100">
                        <div
                          ...
                          style={{ width: ratioWidth(ratioValue as string) }}
                        />
                      </div>
                    </div>
                  ))}
          ...
          {activeTab === "loss-model" && (
            <Section
              title="损耗模型影响"
              description="损耗模型是基于花材可用率的经营估算，不等同于真实报损。该分析用于判断哪些花材最容易吃掉利润。"
            >
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                <StatCard
                  label="原始花材成本"
                ...
                <StatCard
                  label="损耗调整后花材成本"
                  value={formatCurrency(data.lossModelImpact.lossAdjustedFlowerMaterialCost)}
                />
                <StatCard
                  label="损耗模型增加成本"
                ...
                <StatCard
                  label="损耗成本占销售额"
                  value={formatPercent(data.lossModelImpact.lossModelExtraCostRatioToSales)}
                />
              </div>
              <div className="mt-4">
                    ...
                    {data.lossModelImpact.topMaterialsByLossImpact.length === 0 ? (
                      ...
                      data.lossModelImpact.topMaterialsByLossImpact.map((item) => (
                        <tr key={item.flowerWikiId} className="hover:bg-zinc-50/50">
                          ...
                          <td className="px-4 py-3">{formatCurrency(item.lossAdjustedCost)}</td>
                          <td className="px-4 py-3 text-amber-700">
                            {formatCurrency(item.lossModelExtraCost)}
                          </td>
                          <td className="px-4 py-3">
                            {item.avgUsableRate ? formatPercent(item.avgUsableRate) : "—"}
                          </td>
                        </tr>
                      ))
                    )}
